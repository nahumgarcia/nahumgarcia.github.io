...
- **Grid de `/fotos/`:** `.photo-grid`, CSS Grid con `grid-template-columns: repeat(auto-fill, minmax(255px, 298px))`, `align-items: center` y `gap: 12px` — fotos a su ratio natural, ocupa todo el ancho del navegador (se sale de `.site-outer` con el truco `100vw` + márgenes negativos) con `padding` propio a los lados (2rem desktop/tablet, 1rem móvil). **Ojo:** al fijar un máximo en píxeles (298px) en vez de `1fr`, las columnas ya no se estiran para llenar el ancho sobrante de la fila — puede quedar un hueco en blanco a la derecha de algunas filas según el ancho de ventana, ya que `auto-fill` calcula cuántas columnas caben usando el mínimo (255px) pero cada una solo crece hasta el máximo fijo, sin repartirse el resto. Antes (`minmax(170px, 1fr)`) esto no pasaba porque `1fr` sí absorbía el espacio sobrante.
