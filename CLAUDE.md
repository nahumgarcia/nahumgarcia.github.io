...
- **Orden del contenido en `photo_post.html` y `photo_wall.html`:** título, luego el contenido/descripción (si lo hay), y al final fecha + tags (`.photo-post-postmeta`/`.photo-wall-postmeta`, ambas reusando la clase compartida `.post-meta`). El título (`.photo-post-title`/`.photo-wall-title`) usa `font-size: inherit` (mismo tamaño que el cuerpo del texto, no un tamaño de titular) con `font-weight: 600` para diferenciarse solo por peso, no por tamaño. El espacio entre título/contenido/fecha+tags es de `0.75rem` en el post individual y `0.5rem` en el Muro (`margin-top` de `.photo-post-caption`/`.photo-post-postmeta` y `.photo-wall-caption`/`.photo-wall-postmeta` respectivamente) — deliberadamente poco, para que se lea como un bloque compacto de metadatos, no como secciones separadas.
...
**El redimensionado ocurre automáticamente en el build**, no al subir la foto — así funciona igual sin importar si la foto se sube desde el Mac o desde el móvil (que es todo el sentido de usar Pages CMS). El workflow `.github/workflows/jekyll.yml` tiene un paso "Resize photos for web" antes de `jekyll build` que instala ImageMagick (`apt-get install imagemagick` — **no viene preinstalado en el runner `ubuntu-latest`**, a pesar de que durante un tiempo sí lo estuvo; si este paso falla con `mogrify: command not found`, es que GitHub ha vuelto a quitarlo de la imagen y hay que revisar que el `apt-get install` siga ahí) y luego recorre `assets/photos/` generando **dos variantes** de cada foto, siempre a partir del original (nunca de una copia ya reducida):

- **Miniatura** (`assets/photos/thumb/<archivo>`, `convert -resize "600x600>" -quality 75`): para vistas en cuadrícula donde la foto se ve pequeña — Archivo (`photo_page.html`), Álbumes (portadas en `albums_page.html`/`album_pages.rb`, y la cuadrícula de cada álbum en `album_page.html`) y la fila de fotos de la home (`home_feed.html`). 600px cubre con margen la pantalla retina de esas cuadrículas (mínimo 170px en Archivo, 280px en Álbumes) sin arrastrar el peso de una foto a tamaño completo — una cuadrícula de 24 fotos pasa de tener que decodificar ~200MB a ~20MB.
- **Completa** (`assets/photos/<archivo>`, el propio original reducido in-place con `mogrify -resize "2000x2000>" -quality 82`): para donde sí se ve la foto grande — Muro (`photo_wall.html`), el post individual de una foto (`photo_post.html`) y el lightbox (`data-full`). 2000px es 2x el ancho máximo de visualización (1000px), suficiente para pantallas retina.

En las plantillas Liquid, la ruta de la miniatura se deriva de `photo_src` (ya resuelto a `/assets/photos/<archivo>`) quedándose solo con el nombre de archivo e insertando `thumb/` delante: `{% assign thumb_src = "/assets/photos/thumb/" | append: (photo_src | split: "/" | last) %}`. En Ruby (`album_pages.rb`, que calcula la portada de cada álbum fuera de Liquid) se hace el mismo cálculo a mano y se guarda en `cover_thumb_url` (separado de `cover_url`, que sigue apuntando a la versión completa por si se necesita en el futuro).

Ambos pasos solo modifican la copia efímera del runner que se despliega — **los originales en el repo se quedan intactos**, sin necesidad de que el workflow haga commit de vuelta (evita el riesgo de bucles de commits automáticos y mantiene el histórico limpio).
...
- **Grid de `/fotos/`:** `.photo-grid`, CSS Grid con `grid-template-columns: repeat(auto-fill, minmax(220px, 1fr))`, `align-items: center` y `gap: 12px` — el número de columnas se ajusta solo según el ancho disponible (nunca por debajo de ~220px por foto), fotos a su ratio natural, ocupa todo el ancho del navegador (se sale de `.site-outer` con el truco `100vw` + márgenes negativos) con `padding` propio a los lados (2rem desktop/tablet, 1rem móvil). El máximo se deja en `1fr` a propósito (no un valor fijo en píxeles) para que las columnas sigan estirándose y llenen el ancho de la fila sin dejar huecos — se probó con un máximo fijo (`minmax(255px, 298px)`) y dejaba hueco en blanco al final de algunas filas, porque `auto-fill` calcula cuántas columnas caben usando el mínimo pero luego no reparte el sobrante si el máximo no es flexible.
...
- **Padding lateral en mobile (`max-width: 600px`):** reducido a `1rem` (16px) en `.site-outer` y `.photo-grid` (antes 1.5rem/24px, igual que en desktop). `.post-content img`, que calcula su ancho a pantalla completa con `calc(100vw - Xrem)`, tiene su propio override a `2rem` en mobile para que sus bordes queden alineados con el nuevo margen del texto (en desktop sigue restando `3rem`, acorde al padding de `.site-outer` sin reducir). `.site-header` es la excepción: solo reduce su `padding-left` a `1rem` en mobile, pero el `padding-right` sube a `2rem` (32px) — el estándar de 16px no alcanza para el selector de tema (`.theme-toggle`, que flota más allá del borde derecho del nav vía `left: 100%` + `margin-left`), porque el icono solo ya ocupa 16px: con 16px de padding no cabe margen hasta el texto del nav NI hueco hasta el borde a la vez. Con 32px de padding y `margin-left: 0.5rem` (8px) en el toggle, quedan 8px simétricos a cada lado del icono (8px desde "Sobre mí" y 8px hasta el borde) — se probó con `margin-left: 0` (icono pegado al texto del nav) y por separado con el padding en 24px (icono pegado al borde), ambos casos se sienten "pegados" y no es solo cuestión de gusto: con un icono de 16px y solo 24px de padding disponibles, matemáticamente no hay sitio para dos huecos a la vez.
