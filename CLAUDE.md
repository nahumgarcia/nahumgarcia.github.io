...
- **Grid de `/fotos/`:** `.photo-grid`, CSS Grid con `grid-template-columns: repeat(auto-fill, minmax(170px, 1fr))`, `align-items: center` y `gap: 12px` — el número de columnas se ajusta solo según el ancho disponible (nunca por debajo de ~170px por foto), fotos a su ratio natural, ocupa todo el ancho del navegador (se sale de `.site-outer` con el truco `100vw` + márgenes negativos) con `padding` propio a los lados (2rem desktop/tablet, 1rem móvil)
