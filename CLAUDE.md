...
- **Enlaces sin affordance visual en dispositivos táctiles:** enlaces que se distinguen solo por color (TOC, `.item-meta a`, `.lightbox-permalink`, cabeceras de sección de la home, `.home-index-link`, nav (`.site-nav a`), footer (`.site-footer a`)) se subrayan bajo `@media (hover: none)`, ya que sin `:hover` no hay otra pista de que son clicables. Pills, botones y paginación no lo necesitan porque ya tienen su propio look de "esto es interactivo".
